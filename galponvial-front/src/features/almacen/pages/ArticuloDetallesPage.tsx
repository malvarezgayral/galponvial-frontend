      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
            ...
            <div>
                ...
                <p className="text-gray-500 text-sm">Código Interno: {articulo.cod}</p>
            </div>
        </div>
        
        <div className="flex gap-2">
            <Button variant="primary" onClick={() => console.log('Editar click')}>
                Editar
            </Button>
        </div>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div className="md:col-span-1">
                <div className="aspect-square bg-gray-200 rounded-lg flex items-center justify-center text-gray-400">
                    {/* FUTURA LÓGICA DE IMAGEN PARA CUANDO HAYA*/}
                ...
                <div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8">
                        ...
                        <div className="border-b border-gray-100 pb-2">
                            <span className="text-gray-500 text-sm block">Código Proveedor</span>
                            <span className="text-gray-900 font-medium text-lg">{articulo.cod_proveedor || '-'}</span>
                        </div>

                        <div className="border-b border-gray-100 pb-2">
                            <span className="text-gray-500 text-sm block">Modelo</span>
                            <span className="text-gray-900 font-medium text-lg">{articulo.modelo || '-'}</span>
                        </div>

                        <div className="border-b border-gray-100 pb-2">
                            <span className="text-gray-500 text-sm block">Tipo de Unidad</span>
                            <span className="text-gray-900 font-medium capitalize">{articulo.unidad_tipo}</span>
                        </div>
                    </div>
                </div>

                <div className="bg-blue-50 rounded-lg p-4 border border-blue-100">
                    <h3 className="text-blue-800 font-semibold mb-1">Stock Actual</h3>
                    <div className="flex items-baseline gap-2">
                        <span className="text-4xl font-bold text-blue-900">{articulo.stock}</span>
                        <span className="text-blue-700 font-medium capitalize">{articulo.unidad_tipo}s</span>
                    </div>
                        ...
                        {movimientos.map((mov, idx) => (
                            <tr key={idx} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900 font-medium">
                                    {typeof mov.dniUsuario === 'object' ? mov.dniUsuario : mov.dniUsuario}
                                </td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                    {mov.motivo}
                                </td>
