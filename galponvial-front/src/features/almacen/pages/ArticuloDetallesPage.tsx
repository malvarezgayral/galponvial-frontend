...
const ArticuloDetallesPage: React.FC = () => {
  ...
  const refetchAllData = useCallback(async () => {
    ...
    try {
      ...
      const error =
        err instanceof Error ? err : new Error("Error al cargar el artículo");
  ...
  const getMovimientoBadgeColor = (tipo: string) => {
    ...
    if (tipoLimpio.includes("entrada") || tipoLimpio.includes("alta"))
      return "bg-green-100 text-green-800";
    if (tipoLimpio.includes("salida") || tipoLimpio.includes("baja"))
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
          ...
          <Button variant="primary" onClick={() => navigate("/almacen")}>
            Volver al Almacén
          </Button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg shadow p-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          <div className="md:col-span-1">
            <div className="md:col-span-1">
              <div className="aspect-square bg-gray-100 rounded-lg flex items-center justify-center text-gray-400 border border-gray-200 overflow-hidden relative">
                {articulo.img_url ? (
                  <img
              ...
              <div className="mb-8">
                <div className="bg-blue-50 rounded-lg p-4 border border-blue-100 w-fit min-w-50">
                  ...
                  <div className="flex items-baseline gap-2">
                    ...
                    <span className="text-blue-700 font-medium capitalize text-sm">
                      {articulo.unidad_tipo}s
                    </span>
                  </div>
            ...
            <div className="mt-auto pt-6 border-t border-gray-100">
              ...
              <p className="text-gray-900 text-base leading-relaxed">
                {articulo.descripcion ||
                  "No hay una descripción detallada para este artículo."}
              </p>
            </div>
          </div>
        </div>
                ...
                {movimientos.map((mov, idx) => (
                  <tr key={idx} className="hover:bg-gray-50 transition-colors">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <Badge
                        variant="primary"
                        className={getMovimientoBadgeColor(mov.tipoMovimiento)}
                      >
